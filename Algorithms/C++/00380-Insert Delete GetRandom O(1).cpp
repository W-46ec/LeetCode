...
class RandomizedSet {
    // arr stores the actual data
    vector<int> arr;
    // val2idx maps from values to indices in the array
    map<int, int> val2idx;
public:
    RandomizedSet() {}
    ...
    bool remove(int val) {
        ...
        if (val2idx.find(val) != val2idx.end()) {
            ...
            arr[val2idx[val]] = last_element;
            val2idx[last_element] = val2idx[val];
            val2idx.erase(val);
            arr.pop_back();
            return true;
        }
...
int main(int argc, char *argv[]) {
    ...
    cout << "obj.insert(2): " << obj.insert(2) << endl;
    cout << "obj.getRandom(): " << obj.getRandom() << endl;
