...
class Solution:
    def maxSubArrayLen(self, nums: List[int], k: int) -> int:
        # Contruct a prefix sum array such that prefixSum[i]
        # is the sum of the first i elements in nums
        prefixSum, S_i = [], 0
        for x in nums:
            S_i += x
            prefixSum.append(S_i)
        
        # Now the problem is similar to the Two Sum problem:
        ...
        for idx, val in enumerate(prefixSum):
            ...
            # Store the current value in the hashmap so that
