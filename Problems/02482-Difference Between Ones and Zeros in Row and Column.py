...
class Solution:
    def onesMinusZeros(self, grid: List[List[int]]) -> List[List[int]]:
        ...
        diff_row = [2 * sum(row) - n for row in grid]
        diff_col = [2 * sum(col) - m for col in zip(*grid)]
...
class Test(unittest.TestCase):
    ...
    def testcase1(self):
        self.assertEqual(self.soln_obj.onesMinusZeros([[0, 1, 1], [1, 0, 1], [0, 0, 1]]), [[0, 0, 4], [0, 0, 4], [-2, -2, 2]])

    def testcase2(self):
        self.assertEqual(self.soln_obj.onesMinusZeros([[1, 1, 1], [1, 1, 1]]), [[5, 5, 5], [5, 5, 5]])
