...
class Solution:
    def myAtoi(self, str: str) -> int:
        ans, lst = 0, list(filter(lambda x: len(x) > 0, str.strip().split(' ')))
        if len(lst) > 0:
            idx = 1 if lst[0][0] == '-' or lst[0][0] == '+' else 0
            while idx < len(lst[0]) and lst[0][idx].isnumeric():
                idx += 1
            w = lst[0][ : idx]
            ans = int(w) if idx > 0 and w != '-' and w != '+' else 0
        if ans >= 2 ** 31:
            return 2 ** 31 - 1
        elif ans < -(2 ** 31):
            return -(2 ** 31)
