	...
	def largestMultipleOfThree(self, digits: List[int]) -> str:
		...
		def solve(arr, ans):
			...
			if sum(arr) % 3 != 0:
				...
				if len(s) > 0 and int(s) > ans[0]:
					ans[0] = int(s)
		solve(digits, ans)

		return "" if ans[0] == -1 else str(ans[0])
