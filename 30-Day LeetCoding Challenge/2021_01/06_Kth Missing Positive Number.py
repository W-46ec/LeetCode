...
class Solution:
    def findKthPositive(self, arr: List[int], k: int) -> int:
        # Using set
        missing = sorted(list(set(range(1, arr[-1] + 1)) - set(arr)))
        return missing[k - 1] if k - 1 < len(missing) else arr[-1] + k - len(missing)
    
        # # Using loop
        # missing, i, curr_idx = [], 1, 0
